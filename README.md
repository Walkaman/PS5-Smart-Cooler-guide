...
| **GPIO2** | PWM_DIAG_PIN соединяется с выводом PWM OUT (диагностический вывод, подключать не обязательно) | Вход для измерения высокого уровня ШИМ-сигнала на вентиляторе. Используется для диагностики: отображает напряжение в Advanced Settings. |
