...
- [2. ПОДКЛЮЧЕНИЕ И НАСТРОЙКА](#02)
  ...
  - [2.2.1 Примеры установки на PS5 Slim 1 rev](#02-02-01)
...
- [3. ИНСТРУКЦИЯ ПО ЭКСПЛУАТАЦИИ](#03)
  ...
  - [3.6 Верхние значки (системные настройки)](#03-06)
    ...
    - [3.6.9 Диагностика уровня PWM-сигнала](#03-06-09)
...
| **GPIO2** | PWM_DIAG_PIN соединяется с выводом PWM_OUT доп. платы (диагностический вывод, подключать не обязательно) | Вход для измерения высокого уровня ШИМ-сигнала на вентиляторе. Используется для диагностики: отображает напряжение в Advanced Settings. |
...
![image](media/installationexample8.png)

*Температуры до и после установки PS5 Smart Cooler(профиль управления вентилятора 1-базовый андервольт, 2-управление вентилятора по кривой, 3-стабилизация температур и оборотов)*
...
#### <a id="03-06-09"> 3.6.9 Диагностика уровня PWM-сигнала

Устройство оснащено встроенной функцией диагностики уровня сигнала ШИМ (PWM) на выходе вентилятора. Это позволяет быстро проверить, корректно ли формируется управляющий сигнал и достаточен ли его уровень напряжения (Вывод GPIO2 Должен быть соединен с выводом PWM_OUT доп. платы)
...
Дополнительная плата из комплекта предназначена для переключения сигнала управления вентилятором от консоли и от девайса (выбор между источником управления вентилятором), так же с помощью доп платы осуществляется функция безопасности: если девайс выйдет из строя или вдруг зависнет, управление кулерои автоматически переключится на штатное от консоли (без доп платы регулировка кулера, мониторинг и андервольт будет работать)
...
**Вопрос:** Можно ли использовать ESP32-C3 без дополнительной платы мультиплексора?  
**Ответ:** Да, для режима мониторинга и адеррвольта (только чтение температуры, настройки напряжения и скорости вентилятора) мультиплексор не нужен. Однако для полноценного управления вентилятором с возможностью переключения между управлением от PS5 или PS5 Smart Cooler требуется дополнительная плата с коммутацией PWM.
